...
class Game:
	...
	def playOneGame(self, p1, p2, show):
		...
		while True:
			...
			print("Move Selection Time: %s seconds" % (time.time() - start_time))
			if move == []:
				print("Game over")
				return 'W'
			try:
...
class AlphaBetaPlayer(Game, Player):
	...
	def createChild(self, board):
		child = AlphaBetaPlayer(self.size)
		childSide = self.opponent(self.side)
		child.initialize(childSide, self.depth - 1, board)
		return child
...
AlphaBetaPlayer.getEvaluation = MinimaxPlayer.getEvaluation = getEvaluation
AlphaBetaPlayer.getEvaluation1 = MinimaxPlayer.getEvaluation1 = getEvaluation1
AlphaBetaPlayer.getEvaluation2 = MinimaxPlayer.getEvaluation2 = getEvaluation2

if __name__ == '__main__':
	game = Game(8)
	player1 = AlphaBetaPlayer(8)
	player1.initialize('B', 4)
